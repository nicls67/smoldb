...
fn basic_ops_1() -> Result<(), String> {
    ...
    check_option(
        (6, 1),
        table.get_entry_value_string(&"new_entry_name".to_string(), &"key1".to_string())?,
        false,
    )?;
...
fn error_ops_1() -> Result<(), String> {
    ...
    check_result(
        (1, 1),
        db.database().table(&"Table test 1".to_string()),
        false,
    )?;
    ...
    check_result(
        (2, 1),
        table.get_entry_value_float(&"entryX".to_string(), &"key3".to_string()),
        false,
    )?;
    ...
    check_result(
        (2, 2),
        table.get_entry_value_float(&"entry1".to_string(), &"key4".to_string()),
        false,
    )?;
